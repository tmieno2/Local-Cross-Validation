...
$$
y_i = f(N_i, alpha_i, \beta_i, Nk_i) + v_i
$$

For both RF and BRF, the number of trees are set at .

Once $f(\cdot)$ is estimated (denoted as $\hat{f}(\cdot)$), then the following problem is numerically solved to identify EONR specific to site $i$ for all $i = 1, \dots, N$.
...
CF approach in this study largely follows @kakimoto2022causal. Four treatments are defined where the lowest rate (N_1) serves as the base line: $N_2$-$N_1$, $N_3$-$N_1$, $N_4$-$N_1$, and $N_5$-$N_1$. The impact of each treatment is then estimated via multi-arm causal forest simultaneously using the R grf package. Let $\phi_k$ denote $E[y|N_k] - E[y|N_1]$, namely the expected impact of applying $N_k$ relative to $N_1$. Then, the profit impact for applying $N_k$ instead of $N_1$ is $P_C \cdot \hat{\phi}_k - P_N \cdot (N_k - N_1)$. Among the series of rates, the rate at which the profit impact is maximized is chosen as the EONR for each site $i$. When all of the profit impacts are negative, then the $N_1$ (the lowest) rate is chosen as the EONR.
...
## GWR-estiamted Local EONR vs true EONR
