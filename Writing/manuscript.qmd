...
Based on the experimental design, response parameters ($\mathbf{\theta}_{j,i}$), and error term $\varepsilon_{j,i}$, the yield $y_{j, i}$ for each cell $(j, i)$ was generated following the true response function in @eq-yield-response. However, as mentioned earlier, the harvesting equipment was assumed to be able to accurately collect yield data only at the 18m $\times$ 12m (subplot) resolution. Therefore, the observed yield data were at the resolution of subplot level, denoted as $y_j$ ($j=1, \ldots, 1440$). Here we assumed $y_j$ is the average yield for the six cells within the subplot $j$. The observed yield map for the 1440 subplots in one example simulation was displayed in @fig-Nrate-yield (bottom panel).
...
Our findings highlight that model selection should be aligned with the ultimate decision objective rather than the prediction tasks. When the goal is to provide profitable input recommendations, evaluating models based on their ability to estimate economically relevant outcomes is more appropriate than evaluating them based on yield prediction accuracy.

Future research may extend this framework by exploring alternative methods for estimating local EONR, incorporating additional sources of uncertainty, and applying the approach to real world on-farm precision experiment data.
