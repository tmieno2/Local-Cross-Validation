...
Consequently, the true yield response function has three degrees of freedom (five parameters across two equations). This implies that once three of the five parameters are identified, the remaining two are automatically determined. Naturally, none of the candidate models are explicitly aware of this structural relationship. It is their task to uncover it during the training process, except in the case of linear models that impose a specific functional form by assumption.
...
Unlike in the LM and SE approaches, in the random forest (cite) and boosted regression forest (cite) approaches, yield response functions are non-parametrically estimated without  functional form assumptions. These approaches are well known to more accurately predict yield **levels** than do traditional linear models, and their ability to estimate the marginal impact of input on crop yield can be questionable (cite).

Letting $\widehat{f}(N, X)$) denote the estimated yield response function from either of the approaches, variable rate EONRs are calculated numerically by solving the following profit maximization problem at each site ($i$) of the field:
...
Now let $m^{s}$ and $\widehat{N}^*_{m^s}(X_i)$) denote the model selected by approach $s$ and the EONR for cell $i$ estimated based on the model. Then the profit achieved by this model is:
