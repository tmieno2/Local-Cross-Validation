...
$$
\hat{N}_i =  argmax_{N} P_c \widehat{f(N, X|\Sigma)} - P_N N
$$
...
Let $\widehat{N}^*_{m}(X_i)$ denote the estimated EONR for cell $i$ based on the trained model $m$. Hereafter, the collection of EONR for all the cells are called simpley site-specific N prescription. The average profit per acre associate with the site-speicifc N prescription by model $m$ (denoted as \pi_m) is

\begin{align}
\pi_m  = \sum_{i=1}^{N} P_c \cdot f(\widehat{N}^*_{m}(X_i), X_i) - P_N \cdot \widehat{N}^*_{m}(X_i) / N
\end{align}

## True model performance in estimating Site-Specific EONR

The main goal of the MC simualtions is to assess the ability of the two model selection procedures (based on local-EONR and yield) in estimating site-specific EONR. To achieve this simulation goal, it is necessary to identify the best performing model under the circumstance replicating the actual situation researchers faces: that is estimate models using the data from the whole field and estimate site-specific EONR. The ranking of the performance of the candidate models serve as the reference point to be compared against the ranking of their models suggested by the two model selection procedures. For example, suppose our LE-selection method suggests RE and Y-selection method suggests BRT as the top-performing model based on their own criteria. Further suppose that the process described here suggests BRT as the best performing model 

+ train data: all observations of the field
+ target data: all observations of the field

For a given field, we take the following steps for each of the candidate models:
...
\item compare the estimated site-speicific EONR with the true EONRs and calculate RMSE of EONR estimation
\end{enumerate}

Once these steps are completed, then the candidate models are ranked based on their RMSE of EONR estimation. This process is repeated for all the fields. Note that step 3 of the above process compares the estimated EONR with the \textit{true} EONR and the obtained RMSEs represent the true performance of the models' ablity to estiamte site-specific EONR. This is possible only because we know the underlying site-speicifc yield reponse functions. A model selection method that is capable of selecting the true best model more often is considered a better one.


$P_c f(\widehat{N}^*_{m_f}(X_i), X_i) - P_N \widehat{N}^*_{m_f}(X_i)$
...
### RMSE of EONR estiamtion and profit loss

We use two indicators for models' ability to estiamte site-specific EOIR. First measure is the RMSE of site-speific EONR estimation. The second measure is the profitability associated with the site-specific EONR estiamted by the model.

The profit deficit $\Delta \pi_i$ can be calculated as follows:
...
We implement two model selection approaches: one based on yield preidction accuracy and the other based on local EONR prediction accuracy. For both approaches, spatial cross-validation is employed. The entire dataset is split into a train and test datset in a spatially clustered manner. This process is repeated to create multiple folds (a set of spatially clustered train and test datasets) as depcited in Figure \ref{}. 
...
Once the above process is completed for all the folds, then average RMSE-Yield was calculated for each model and the model with the lowest average RMSE value. 

### Model selection based on local EONR 

In validating models, it would be ideal if we could assess models based on their ability to estimate site-specific EONRs rather than yields because our ultimate interests lie in estimating EONRs, not yields. Of course, this is not possible because the true EONRs are never observable in the real world setting. We propose to overcome this challenge by validating on "local uniform EONR" estimated by a statistical model. Specifically, we train a GAM model using the test data and predict the uniform EONR for the test data, which we call local EONR. We then treat the estimated local EONR as the proxy for the true local EONR and select models that are capable of predicting the estimated local EONRs the best. Clearly, for this approach to perform well, it is vital that the estimation of local EONR is accruate enough (Later we assess the accuracy).

For each of the fold, the following steps are implemented to estiamte local EONR:
...
Once the above step process is completed for all the folds, then the RMSE of estiamting local EONR is calculated for each model as follows:

\begin{align}
RMSE-LEONR_{m} = \sqrt{\sum_{f=1}^F (\widehat{N^*_{f,m}} - \widehat{N^*_{f,gam}})^2 / F}
\end{align}
...
Two criterion to evaluate model selection performace are used. First, they are evaluated based on the percentage of the times the selection approach picked the true best model in estimating EONR. The second and more practically informative criteria is based on loss in profit when the selected model was used instead of the true best model.  


For example, suppose a selection approach 
...
averaged_df <- main_sum_results %>%
  group_by(sim, method) %>%
  summarize(
    avg_rmse_eonr_true = mean(rmse_eonr_true),
