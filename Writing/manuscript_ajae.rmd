...
The R-learner is a machine learning technique used in causal inference to estimate treatment effects. 
...
There are variations in profit outcomes when calculating local EONRs using candidate ML models. To determine the profit deficits associated with each model, we establish reference points based on the actual optimal yield and actual optimal nitrogen levels. These reference values are derived through simulation and serve as benchmarks for comparison.
...
Furthermore, this comparison serves as a critical reference point during our model selection process on the folds. The model selected based on this evaluation is considered the "true best model" as it demonstrates its proficiency in estimating site-specific EONRs on the dataset used for both training and validation.
...
After calculating the root mean square error (RMSE) of the true site-specific EONRs for candidate ML model across the entire set of 500 fields, the next step involves ranking these models based on these evaluation metrics. By comparing the profit deficits and RMSE of the estimated site-specific EONRs with the actual EONRs, we can identify the model that exhibits superior performance in accurately predicting site-specific EONRs. A lower RMSE signifies a closer match between the model-predicted site-specific EONRs and the actual EONRs. 

## Model Selection based on Local EONRs (stream 2)

We propose an approach for model selection based on local EONR values.

Our approch: spatial cross-validation of local EONR estimation

+ true site-specific EONR is not observed. that's why we estiamte Local EONR (why not estimate site-specific EONR?)

+ Dividing the whole data into multiple folds in a spatially clustered manner (example figure)
  + This technique enables us to assess the robustness and stability of the obtained clusters.
  + We then performed an analysis to identify and exclude folds and repetitions that exhibited significant similarities. (specific)
+ For each fold we applied GAM to the test data to estiamte yield response function, and then obtain uniform EONR for that fold, which we call estiamted local EONR
+ For each fold, we used the train data to train candidate ML models and then estimate site-specific eonr for observations in the test data
+ Compare GAM-estimated and ML-estimated local EONR and calculate the RMSE (ranking)

### Estimate Local EONR Using GAM a Proxy for True EONR
...
Let ${\hat{g}}_{j\varphi GAM}(N_j)$ represent the yield response function estimated by GAM for the sequence of nitrogen rates $N_j$ at split $\varphi$. In order to estimate gam local EONRs for each split, we solve the following problem for every split $\varphi$, and nitrogen level $N_j$:
...
Subsequently, we compute the RMSE between the gam estimated local EONRs and the actual EONRs within each simulated fold:

$$
RMSE \text { of Local GAM EONR vs Actual EONR }=\sqrt[2]{\frac{1}\phi \sum_{i=1}^\phi\left(\widehat{N}_{\varphi G A M}^{\text {opt }}-N_{\varphi}^{\text {opt }}\right)^2}
$$
...
Then to obtain local EONRs, site-speicfic EONRs were average for each split:
...
### Ranking the ML models by their RMSE between estimated local EONRs and gam
estimated local EONRs

In order to assess the performance of candidate ML models $m$, a comparative analysis is conducted between the model's estimated local optimal nitrogen rates and the locally estimated EONRs derived from GAM within each field. This evaluation utilizes the RMSE metric as a quantitative indicator (rewrite this one).
...
We rank the ML models based on their respective RMSE values of the local EONRs. A lower RMSE indicates a better alignment between the model's estimates and the gam estimates. Thus, we rank the ML models that exhibit lower RMSE values as they demonstrate higher accuracy in predicting the local EONRs.

## Model Selection based on yield prediction ability (Stream 3)

Everybody uses yield-prediciotn capability as their model selectio criteria. We also rank models based on yield-prediciotn capability and compare its performance with our model selection method in terms of EONR estimation accuracy.
