...
```{r}
find_local_EONR_Linear <- function(data,test_data, linear){
  N_data <- data.table(N_tgt = seq(min(data$N_tgt), max(data$N_tgt), by = 1))
  X_test <-
    test_data[, c(cov_list, "aunit_id"), with = FALSE] %>%
    .[, N_tgt := NULL] %>%
...
```{r}
find_local_EONR_gam <- function(data,test_data, gam){
...
gam <- gam(yield ~ s(N_tgt, k=3), data = test_data)
  ...
  # /*+++++++++++++++++++++++++++++++++++
  opt_EONR_Linear <-
    find_local_EONR_Linear(data,test_data, linear) %>%
    .[, .(N_tgt = mean(N_tgt))] %>%
  ...
  # /*+++++++++++++++++++++++++++++++++++
  opt_EONR_gam <-
    find_local_EONR_gam(data,test_data, gam) %>%
    .[, .(N_tgt = N_tgt)] %>%
...
All_EONR <- lapply(1:60, function(x) find_local_EONR_all(x, spatial_folds, data)) %>%
     rbindlist()
