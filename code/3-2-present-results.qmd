...
main_accuracy_table <-
  ...
  set_header_labels(
    values = list(
      method = "Method",
      num_selected_true = "Best-performing model",
      leonr_based = "Local-EONR selection",
      yield_based = "Yield selection"
    )
  ) %>%
  autofit() %>%
...
g_ranking <-
  ...
  .[, select_type := case_when(
    variable == "num_selected_true" ~ "Best-performing model",
    variable == "num_selected_leonr" ~ "Local-EONR selection",
    variable == "num_selected_yield" ~ "Yield selection"
  )] %>%
  .[, select_type := factor(select_type, levels = c("Best-performing model", "Local-EONR selection", "Yield selection"))] %>%
  ggplot(.) +
  geom_bar(
...
ggsave(
  "writing/figures-tables/g_ranking_presentation.png",
  g_ranking_presentation,
  width = 6,
  height = 4,
  dpi = 400
...
profit_loss_data <-
  ...
  .[, selection := case_when(
    variable == "e_pi_loss" ~ "Local-EONR-based Selection",
...
g_pi_loss <-
  ...
  xlab("Profit loss relative to the most profitable candidate model ($/acre)") +
  theme_bw()
)
...
ggsave(
  here::here("writing/figures-tables/g_pi_loss_presentation.png"),
  g_pi_loss_presentation,
  dpi = 300,
  width = 6,
  height = 4
...
gam_leonr_fit_main <-
  ...
  xlim(50, 300) +
  ylim(50, 300) +
  ylab("True Local EONR (kg/ha)") +
  xlab("GAM-estimated Local EONR (kg/ha)") +
  theme_bw() +
  theme(
    legend.position = "bottom"
  )
...
g_sensitivity_fold_repeat <-
  ...
  labs(
    x = "True ranking of the selected model (1 = best)",
...
#--- first simulation ---#
whole_data <-
  readRDS(here::here("data/main/individual-fields/sim_data_1.rds"))$data[[1]] %>%
  data.table()
...
ggplot2::ggsave(
  here::here("writing/figures-tables/g_split_example_presentation.png"),
  g_split_example_presentation,
  width = 6,
  height = 3,
  dpi = 300
...
ggplot2::ggsave(
  here::here("Writing/figures-tables/g_split_example_10_folds.png"),
  g_split_example_10_folds,
  dpi = 300,
  height = 6,
  width = 6
