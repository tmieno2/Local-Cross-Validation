...
```{r}
results_folder <- here::here("results/main")
```

## Data
```{r}
raw_data <- readRDS(here::here("data/main/raw_sim_data.rds"))
...
## Combine the results and calculate profit deficit

```{r}
results_whole <-
  list.files(
    file.path(results_folder, "true-best-model"),
    full.names = TRUE
  ) %>%
  purrr::map(readRDS) %>%
  rbindlist() %>%
...
saveRDS(sum_results_whole, file.path(results_folder, "sum_results_whole.rds"))
```
...
```{r}
sim_cases <- readRDS(file.path(results_folder, "sim_cases.rds"))
...
comp_results <-
  ...
  mutate(comp_summary = list(
    process_model_selection_results(
      num_folds, 
      num_repeats,
      results_root_dir = results_folder
    )
  )) %>%
...
saveRDS(comp_results, file.path(results_folder, "comp_results.rds"))
```
...
gam_results <-
  ...
  dplyr::mutate(gam_results = list(
   list.files(
    paste0(results_folder, "/sim_results_num_repeats_", num_repeats, "_num_folds_", num_folds),
    full.names = TRUE
  ) %>%
  .[str_detect(., "gam")] %>%
  purrr::map(readRDS) %>%
...
sum_results_gam <-
  ...
  # within a fold
  .[, .(opt_N_gam = mean(opt_N_gam), opt_N = mean(opt_N)), by = .(sim, split_id, num_folds, num_repeats)] %>%
  .[, opt_N_dif := opt_N - opt_N_gam] %>%
  .[, rmse_local_eonr := sqrt(mean(opt_N_dif^2)), by = sim] %>%
  .[order(num_folds, num_repeats, sim), ]

saveRDS(sum_results_gam, file.path(results_folder, "sum_results_gam.rds"))
```