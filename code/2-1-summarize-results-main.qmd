...
```{r}
sim_cases <- readRDS(here::here("results/main/sim_cases.rds"))
...
saveRDS(comp_results, here::here("results/main/comp_results.rds"))
```
...
gam_results <-
  ...
  dplyr::mutate(gam_results = list(
   list.files(
    paste0("results/main/sim_results_num_repeats_", num_repeats, "_num_folds_", num_folds),
    full.names = TRUE
  ) %>%
  .[str_detect(., "gam")] %>%
  purrr::map(readRDS) %>%
...
saveRDS(sum_results_gam, here::here("results/main/sum_results_gam.rds"))
```