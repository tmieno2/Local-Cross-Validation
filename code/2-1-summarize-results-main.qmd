...
results_whole <-
  list.files(
    here::here("results/main/true-best-model"),
    full.names = TRUE
  ) %>%
  purrr::map(readRDS) %>%
  rbindlist() %>%
...
saveRDS(sum_results_whole, here::here("results/main/sum_results_whole.rds"))
```
...
```{r}
sim_cases <- readRDS(here::here("Shared/Results/sim_cases.rds"))
...
gam_results <-
  ...
  dplyr::mutate(gam_results = list(
   list.files(
    paste0("Shared/Results/sim_results_num_repeats_", num_repeats, "_num_folds_", num_folds),
    full.names = TRUE
  ) %>%
  .[str_detect(., "gam")] %>%
  purrr::map(readRDS) %>%
...
saveRDS(sum_results_gam, here::here("Shared/Results/sum_results_gam.rds"))
```