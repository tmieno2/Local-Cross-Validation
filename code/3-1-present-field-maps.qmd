...
g_field_map_presentation <-
  ...
  coord_sf(
    xlim = c(fbox["xmin"] - 32, fbox["xmax"] + 5),
    ylim = c(fbox["ymin"] - 5, fbox["ymax"] + 32),
    expand = FALSE
  )

ggsave(
  file = here::here("writing/figures-tables/g_field_map_presentation.png"),
  g_field_map_presentation,
  width = 7,
  height = 3.7,
  bg = "white"
...
# target N rate data
new_levels <- 
  c("transition", levels(gdata$N_tgt %>% factor())) %>%
  unique()
...
ggsave(
  here::here("Writing/figures-tables/g_Nrate.png"),
  g_N_prep,
  width = 7,
  height = 3,
  bg = "white"
...
g_yield_presentation <-
  ...
  guides(fill = guide_colorbar(
    ...
    title = "Observed Corn Yield (kg/ha)",
    title.position = "top", title.vjust = 2
  )) +
  coord_sf(expand = FALSE)

ggsave(
  here::here("Writing/figures-tables/g_yield_presentation.png"),
  g_yield_presentation,
  width = 7,
  height = 4.5,
  bg = "white"
